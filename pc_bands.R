...
region.pc2 <- matrix(NA, nrow(rels.locs), max(pc.want))
counties.pc <- cbind(counties.ppr, pc.res$x[,pc.want])

rels.locs3 <- cbind(rels.locs, region.pc2)
colnames(rels.locs3)[6:(6+max(pc.want)-1)] <- paste0("PC", pc.want)

rels.regs <- st_contains(counties.pc, rels.locs)
for(i in 1:length(rels.regs)){
  rels.locs3[rels.regs[[i]],6:(6+max(pc.want)-1)] <- st_drop_geometry(counties.pc[i,paste0("PC", pc.want)])
}



v = variogram(PC4 ~ 1, rels.locs3)
plot(v)
v.fit = fit.variogram(v, vgm("Mat"))
# v.fit = fit.variogram(v, vgm("Mat", range=4*10^5, nugget=0.45))
plot(v, v.fit)
