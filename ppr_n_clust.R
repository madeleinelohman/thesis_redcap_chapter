...
num.clust <- function(df, pc.res, min.clust, max.clust, pc.want, index, bound_vals, 
  ...
  for(i in min.clust:max.clust){
    cr_q <- redcap(i, rook_w, data, "fullorder-completelinkage", 
                   scale_method="raw",
                   bound_vals, min_bound)
    eval$RS[i-(min.clust-1)] <- cr_q$`The ratio of between to total sum of squares`
    new.clusts <- order.clusts(as.vector(as.integer(cr_q$Clusters)))
    eval$Index[i-(min.clust-1)] <- unlist(intCriteria(as.matrix(data), 
                                                      new.clusts$new, crit=index))
  }
